 ...
 * │ BSD MITM Service                                                        │
 * │   Bind(fd, 10.13.x.x) ──┐                                              │
 * │   Connect(fd, 10.13.x.x)├──► ProxySocketManager                        │
 * │   Send(fd) ──────────────┤      │                                       │
 * │   Recv(fd) ◄─────────────┘      │                                       │
 * │                                 ▼                                       │
 * │                           ┌──────────┐                                  │
...
public:
     ...
     *
     * @param ip Local IP address in host byte order (e.g., 0x0A0D2502 for 10.13.37.2)
     *
     * @note Thread-safe
     */
     ...
     * @return true if ip is in 10.13.0.0/16
     */
    static bool IsLanAddress(uint32_t ip) {
        return ryu_ldn::bsd::IsLanAddress(ip);
    }
     ...
     * @return true if address is in 10.13.0.0/16
     */
    static bool IsLanAddress(const ryu_ldn::bsd::SockAddrIn& addr) {
        return addr.IsLanAddress();
    }
