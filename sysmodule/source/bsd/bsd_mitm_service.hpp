 ...
 * 3. For sockets that bind/connect to LDN addresses (10.13.x.x), we track
...
private:
    ...
    /// MK8DX session mode once detected: 0=unknown, 1=WLAN (LDN ctrl 12345),
    /// 2=LAN (LAN browse 30000). Logged once per session.
    int m_mode = 0;
    /// True if this session's LAN-mode bind triggered
    /// LdnControl::EnterLocalNetworkModePublic(). A LAN-only session never
    /// opens ldn:u, so CloseAccessPoint/CloseStation/Finalize (the only other
    /// callers of ExitLocalNetworkMode) never run for it -- the destructor
    /// must exit local network mode itself when this is true.
    bool m_entered_local_network_mode = false;
