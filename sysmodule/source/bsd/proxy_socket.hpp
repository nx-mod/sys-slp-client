 ...
 * - Tracks local/remote addresses in the virtual 10.13.x.x network
 *
 * ## Data Flow
 *
 * ```
...
#include "proxy_connect_shim.hpp"
...
public:
     ...
     *
     * @note Thread-safe (protected by m_state_mutex, same as the address
     * getters below) -- Bind/Connect run on the MITM IPC thread while the
     * manager's FindSocketByDestination/FindAllSocketsByDestination read
     * this on the tunnel run-loop thread.
     */
    ProxySocketState GetState() const {
     ...
     *
     * @param addr Local address to bind to (must be in LDN network 10.13.x.x)
    ...
    /**
     * @brief Get a copy of the local address
     * @return Local address structure
     *
     * @note Thread-safe (protected by m_state_mutex). Returns by value
     * (rather than the reference this used to return) since the address can
     * be mutated by Bind() concurrently from a different thread than callers
     * reading it -- a reference would let that mutation tear the read.
     */
    ryu_ldn::bsd::SockAddrIn GetLocalAddr() const {
        std::scoped_lock lock(m_state_mutex);
        return m_local_addr;
    }

    /**
     * @brief Get a copy of the remote address
     * @return Remote address structure
     *
     * @note Thread-safe (protected by m_state_mutex). See GetLocalAddr().
     */
    ryu_ldn::bsd::SockAddrIn GetRemoteAddr() const {
        std::scoped_lock lock(m_state_mutex);
     ...
     * @param addr Broadcast address (e.g., 10.13.255.255)
     */
...
private:
    ...
    /**
     * @brief Mutex protecting m_state, m_local_addr, m_remote_addr
     *
     * Bind/Connect/Shutdown/Close mutate these on the MITM IPC thread; the
     * manager's FindSocketByDestination/FindAllSocketsByDestination read them
     * (via GetState/GetLocalAddr/GetRemoteAddr) on the tunnel run-loop
     * thread. Separate from m_queue_mutex, which only protects the rx ring.
     */
    mutable os::Mutex m_state_mutex{false};
     ...
     *
     * Calculated as: local_ip | ~subnet_mask (e.g., 10.13.255.255)
     */
