 ...
 * 10.13.x.x), these sockets need to be intercepted and routed through the
 ...
 *
 * The slp virtual LAN uses the 10.13.37.0/24 subnet:
 * - 10.13.37.2 = This console (default virtual IP, see slp_runtime)
 * - 10.13.37.x = Peer consoles behind the same slp relay
 * - 10.13.37.255 = Subnet broadcast
 *
 * Any socket operation targeting this subnet should be proxied through the
 * tunnel. Broadcast traffic (255.255.255.255) is also proxied so MK8DX LAN
 * mode browse/session broadcasts reach every relay peer.
 *
...
constexpr uint32_t LAN_NETWORK_MASK = 0xFFFFFF00;
 ...
 inline bool IsLanAddress(uint32_t ip) {
     return (ip & LAN_NETWORK_MASK) == LAN_NETWORK_BASE;
 }
...
struct __attribute__((packed)) SockAddrIn {
    ...
    /**
     * @brief Check if this address is in the slp virtual LAN
     */
    /// @gdb{tag="BSD:ALIGN", msg="SockAddrIn::IsLanAddress: this=%p", args="$x0"}
    bool IsLanAddress() const {
        ...
        return ryu_ldn::bsd::IsLanAddress(host_addr);
    }
    ...
    /* Typed accessors -- PREFER THESE.
     *
     * sin_addr always holds NETWORK order (see bsd/ip_addr.hpp). These are the
     * only sanctioned way to read/write it, so the numeric<->network encodings
     * cannot be silently swapped by assigning a bare u32. */
    ::slp::net::IpV4 Addr() const {
        return ::slp::net::IpV4::FromNetwork(sin_addr);
    }
    void SetAddr(::slp::net::IpV4 ip) {
        sin_addr = ip.Network();
    }
};
