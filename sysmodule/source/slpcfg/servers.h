...
#define SLPCFG_USER_MAX 64
#define SLPCFG_PASS_MAX 64

typedef struct {
    char name[SLPCFG_NAME_MAX];
    char host[SLPCFG_HOST_MAX];
    unsigned short port;
    /* Optional relay login. Empty username = no auth configured. A relay that
     * requires a login sends an AUTH_ME (0x04) challenge and forwards nothing
     * to peers that do not answer it. */
    char username[SLPCFG_USER_MAX];
    char password[SLPCFG_PASS_MAX];
} SlpServer;
